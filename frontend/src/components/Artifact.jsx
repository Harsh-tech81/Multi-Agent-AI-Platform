...
function Artifact() {
  ...
  const file = hasArtifacts ? artifacts[0]?.files[selectedFile] : null;
  const htmlContent = hasArtifacts
    ? artifacts[0]?.files?.find((f) => f.name === "index.html")
    : null;
  const cssContent = hasArtifacts
    ? artifacts[0]?.files?.find((f) => f.name === "style.css")
    : null;
  const jsContent = hasArtifacts
    ? artifacts[0]?.files?.find((f) => f.name === "script.js")
    : null;
          ...
          <div className="flex flex-col h-full bg-[#0d0f14]">
            <div className="h-14 px-4 border-b border-white/[0.06] flex items-center gap-3 shrink-0">
              ...
              <div className="flex items-center gap-1 shrink-0">
                ...
                {/* <button
                  className="flex items-center justify-center w-7 h-7 rounded-lg text-slate-500 hover:text-red-400 hover:bg-red-500/10 transition-colors duration-150 bg-transparent border-none cursor-pointer"
                  onClick={handleClose}
        ...
        {mobileOpen && (
          <>
            ...
            <motion.div
              ...
              className="lg:hidden fixed inset-y-0 right-0 z-50 w-[88vw] max-w-[420px] border-1 border-white/[0.06] overhflow-hidden "
            >
      ...
      <AnimatePresence>
        {hasArtifacts && (
          <motion.div
            ...
            animate={{ width: collapse ? 48 : 400, opacity: 1 }}
